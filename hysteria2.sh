...
inst_site(){
    read -rp "请输入 Hysteria 2 的伪装网站地址 （去除https://） [回车默认www.bing.com]：" proxysite
...
insthysteria(){
    ...
    if [[ -f "/usr/local/bin/hysteria" ]] && [[ -f "/etc/hysteria/config.yaml" ]]; then
        yellow "检测到系统已经安装了 Hysteria 2！"
        read -p "是否要重新安装/覆盖配置？[y/N]: " reinstall
...
update_core(){
    ...
    url="https://download.hysteria.network/app/latest/hysteria-linux-$hy_arch"
    green "正在从官方下载核心: $url"
...
menu() {
    clear
    echo "#############################################################"
    echo -e "#                     ${RED}夜未央 Hysteria 2 一键安装脚本${PLAIN}                     #"
    echo "#############################################################"
    echo ""
